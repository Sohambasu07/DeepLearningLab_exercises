...
class CNN(nn.Module):
    ...
    def __init__(self, history_length=1, n_classes=4): 
        ...
        self.conv1 = nn.Conv2d(in_channels=history_length, out_channels=16, kernel_size=3, stride=4, padding=1)
        self.bn1 = nn.BatchNorm2d(num_features=16)
        self.act1 = nn.ReLU() #nn.LeakyReLU(negative_slope=0.2)
        self.cnn_drop1 = nn.Dropout(0.5)
        self.pool1 = nn.MaxPool2d(kernel_size=(2))
        self.conv2 = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=3, stride=2, padding=1)
        self.bn2 = nn.BatchNorm2d(num_features=32)
        self.act2 = nn.ReLU() #nn.LeakyReLU(negative_slope=0.2)
        self.cnn_drop2 = nn.Dropout(0.5)
