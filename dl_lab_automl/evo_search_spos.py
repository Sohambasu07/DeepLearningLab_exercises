...
class EvolutionSearcher(object):
    ...
    def get_random_cand(self):
        ...
        cand_tuple.append(depth_selected)
        cand_tuple.append(config["embed_dim"])
        for i in range(depth_selected):
            cand_tuple.append(config["num_heads"][i])

        for i in range(depth_selected):
            cand_tuple.append(config["mlp_ratio"][i])
            ...
            if random_s < s_prob: # check is depth is mutated
                ...
                #   raise NotImplementedError
                    for i in range(new_depth - depth):
                        num_heads.append(random.choice(self.choices["num_heads"]))
                        mlp_ratio.append(random.choice(self.choices["mlp_ratio"]))
                else:
    ...
    def get_crossover(self, k, crossover_num):
        ...
        def random_func():
            ...
            for i in range(len(p1)):
                output.append(p1[i] if random.random() < 0.5 else p2[i])
            output = tuple(output)
            arch_param_dict = get_arch_param_from_cand(output, self.choices)
            return output, arch_param_dict
