...
class BigramLanguageModel(nn.Module):
    ...
    def sample_random_config(self):
        ...
        config["num_layers"] = random.choice(num_layers)
        config["num_heads"] = [random.choice(num_heads) for _ in range(config["num_layers"])]
        config["embed_dim"] = random.choice(embed_dims)
        config["mlp_ratio"] = [random.choice(mlp_ratio) for _ in range(config["num_layers"])]

        arch_param_dict = self.sampler.sample_epoch(self.get_arch_parameters())

        # arch_param_dict["num_layers"] = torch.zeros(len(num_layers), dtype=torch.int32)
        # arch_param_dict["num_layers"][num_layers.index(config["num_layers"])] = 1
        # arch_param_dict["num_heads"] = torch.zeros(max(num_layers), len(num_heads), dtype=torch.int32)
        # arch_param_dict["num_heads"][:, num_heads.index(config["num_heads"])] = 1
        # arch_param_dict["embed_dim"] = torch.zeros(len(embed_dims), dtype=torch.int32)
        # arch_param_dict["embed_dim"][embed_dims.index(config["embed_dim"])] = 1
        # arch_param_dict["mlp_ratio"] = torch.zeros(max(num_layers), len(mlp_ratio), dtype=torch.int32)
        # arch_param_dict["mlp_ratio"][:, mlp_ratio.index(config["mlp_ratio"])] = 1
        

        # print("num_layers: ", num_layers.index(config["num_layers"]))
        # print("num_heads: ", num_heads.index(config["num_heads"]))
        # print("embed_dim: ", embed_dims.index(config["embed_dim"]))
        # print("MLP ratio: ", mlp_ratio.index(config["mlp_ratio"]))
...
# model = BigramLanguageModel(choices=choices, block_size=128, use_we_v2=True, mixop='spos') #"darts_v1")
