...
class BlipCaption(BlipBase):
    ...
    def greedy_search(
        ...
        while True:
            ...
            # raise NotImplementedError
            next_token_logits = self.text_decoder(input_ids=input_ids, attention_mask=attention_mask, 
            ...
            # print(input_ids.shape, next_tokens.shape)
    ...
    def sampling(
        ...
        while True:
            ...
            # print(next_token_logits.shape)
            soft_probs = F.softmax(topk_logits, dim=-1)
            next_token_indices = torch.multinomial(soft_probs, 1).squeeze(-1)
            next_tokens = torch.gather(topk_indices, 1, next_token_indices.unsqueeze(-1))
            # print(input_ids.shape, next_tokens.shape)
            input_ids = torch.cat([input_ids, next_tokens], dim=-1)
            # END TODO ###################
