...
def train_epoch(epoch, model, dataloader, optimizer, scheduler, loss_fn, device, temperature,
    ...
    for step, batch in enumerate(dataloader):
        ...
        # print(image_feat.unsqueeze(-1).shape, text_feat.unsqueeze(1).shape)
        image_text_sim = (image_feat @ text_feat.T) /temperature
        text_image_sim = (text_feat @ image_feat.T) /temperature
        one_hot = torch.eye(image_text_sim.shape[1], device=image_text_sim.device, 
                            dtype=image_text_sim.dtype)
        it_loss = loss_fn(image_text_sim, one_hot)
        ti_loss = loss_fn(text_image_sim, one_hot)
        loss = (it_loss + ti_loss) / 2
        # END TODO ###################
